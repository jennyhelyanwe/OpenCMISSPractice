...
#=================================================================================#
def EquationsProblemSolverSetUp(problemUserNumber, equationsSet, cellMLOption):
...
def BCCubeSingleFace(solverEquations, dependentField, appliedFace, faceNormal, appliedDirection, increm, optionBC,
    ...
    if option[0] == 2:
        ...
        for i in range(1, numNodes + 1):
            for j in derivFix:
                for component in [1,2,3]:
                    boundaryConditions.AddNode(dependentField, CMISS.FieldVariableTypes.U, 1, j, i,
...
#=================================================================================#
def ExportStressStrain(elements, xiPositions, equationsSet, filename_strain, filename_stress,
    ...
    try:
        file_stress = open(filename_stress, 'w')

    except IOError:
        print 'ERROR: Unable to open ', filename_stress
        return
    ...
    file_stress.write(' Group name: ' + str(groupname_stress) + '\n')
    file_stress.write(' #Fields= 7\n')
    file_stress.write(' 1) element_xi, field, element_xi, #Components=1\n')
    file_stress.write('   1.  Value index= 1, #Derivatives=0\n')
    file_stress.write(' 2) yg1, field, real, #Components=1\n')
    file_stress.write('   1.  Value index= 1, #Derivatives=0\n')
    file_stress.write(' 3) yg2, field, real, #Components=1\n')
    file_stress.write('   1.  Value index= 1, #Derivatives=0\n')
    file_stress.write(' 4) yg3, field, real, #Components=1\n')
    file_stress.write('   1.  Value index= 1, #Derivatives=0\n')
    file_stress.write(' 5) yg4, field, real, #Components=1\n')
    file_stress.write('   1.  Value index= 1, #Derivatives=0\n')
    file_stress.write(' 6) yg5, field, real, #Components=1\n')
    file_stress.write('   1.  Value index= 1, #Derivatives=0\n')
    file_stress.write(' 7) yg6, field, real, #Components=1\n')
    file_stress.write('   1.  Value index= 1, #Derivatives=0\n')
    ...
    if cellMLOption:
        ...
        for i in range(0, len(elements)):
            for j in range(0, len(gaussPoints)):
                file_strain.write(' Node:         '+str(data_pt)+'\n')
                file_stress.write(' Node:         '+str(data_pt)+'\n')
                file_strain.write(' E         '+str(elements[i])+' 3\n')
                file_stress.write(' E         '+str(elements[i])+' 3\n')

                [strain_temp, stress_temp] = equationsSet.StressStrainEvaluateGaussCellML(elements[i],gaussPoints[i])

                for k in range(0,6):
                    file_strain.write('   '+str(strain_temp[k])+'\n')
                    file_stress.write('   '+str(stress_temp[k])+'\n')

                data_pt = data_pt + 1
    else:
        data_pt = 1
        for i in range(0,len(elements)):
            for j in range(0, len(xiPositions)):
                file_strain.write(' Node:         '+str(data_pt)+'\n')
                file_stress.write(' Node:         '+str(data_pt)+'\n')
                file_strain.write(' E         '+str(elements[i])+' 3  '+str(xiPositions[j,0])+'  '+str(xiPositions[j,1])+'  '+str(xiPositions[j,2])+'\n')
                file_stress.write(' E         '+str(elements[i])+' 3  '+str(xiPositions[j,0])+'  '+str(xiPositions[j,1])+'  '+str(xiPositions[j,2])+'\n')
                [strain_temp, stress_temp] = equationsSet.StrainInterpolateXi(elements[i], xiPositions[j,:])
